# -*- coding: utf-8 -*-
"""
Created on Wed Aug 14 20:49:59 2019

@author: jacqueline.cortez

Capítulo 1. Clustering for dataset exploration
Introduction:
    Learn how to discover the underlying groups (or "clusters") in a dataset. By the end of this chapter, you'll be clustering companies 
    using their stock market prices, and distinguishing different species by clustering their measurements.
"""
...
wine_varieties = wine.class_name.values


file = 'fish.csv'
fish = pd.read_csv(file, header=None, names=['varieties', 'f1', 'f2', 'f3', 'f4', 'f5', 'f6'])
fish_samples = fish[['f1', 'f2', 'f3', 'f4', 'f5', 'f6']].values
fish_varieties = fish.varieties.values


file = 'company-stock-movements-2010-2015-incl.csv'
movements = pd.read_csv(file)
movement_samples = movements.drop('Unnamed: 0', axis=1)
movement_varieties = movements['Unnamed: 0']


print("****************************************************")
tema = "3. Clustering 2D points"; print("** %s\n" % tema)
...
print("****************************************************")
tema = "4. Inspect your clustering"; print("** %s\n" % tema)
...
plt.xlabel('Xs')
plt.ylabel('Ys')
plt.title('Clustering 2D points')
plt.suptitle(tema)
...
print("****************************************************")
tema = "6. How many clusters of grain?"; print("** %s\n" % tema)
...
for k in ks:
    ...
    model.fit(grain_samples) # Fit model to samples
...
print("****************************************************")
tema = "7. Evaluating the grain clustering"; print("** %s\n" % tema)

model = KMeans(n_clusters=3) # Create a KMeans model with 3 clusters: model
grain_labels = model.fit_predict(grain_samples) # Use fit_predict to fit model and obtain cluster labels: labels

df = pd.DataFrame({'labels': grain_labels, 'varieties': grain_varieties}) # Create a DataFrame with labels and varieties as columns: df
...
print("****************************************************")
tema = "8. Transforming features for better clusterings"; print("** %s\n" % tema)

model = KMeans(n_clusters=3) # Create KMeans instance: kmeans
labels = model.fit_predict(wine_samples) # Calculate the cluster labels: labels
df = pd.DataFrame({'labels': labels, 'varieties': wine_varieties}) # Create a DataFrame with labels and varieties as columns: df
ct = pd.crosstab(df['labels'], df['varieties']) # Create crosstab: ct
print(ct) # Display ct

scaler = StandardScaler() # Create scaler: scaler
pipeline = make_pipeline(scaler, model) # Create pipeline: pipeline
labels_standarized = pipeline.fit_predict(wine_samples) # Fit the pipeline to samples
wine_samples_standarized = scaler.transform(wine_samples)
df = pd.DataFrame({'labels': labels_standarized, 'varieties': wine_varieties}) # Create a DataFrame with labels and varieties as columns: df
ct = pd.crosstab(df['labels'], df['varieties']) # Create crosstab: ct
print(ct) # Display ct
...
plt.figure()
plt.subplot(1,2,1)
plt.scatter(wine.od280, wine.malic_acid, c=labels, alpha=0.5)
plt.xlabel('od280')
...
plt.subplot(1,2,2)
plt.scatter(wine_samples_standarized[:,11], wine_samples_standarized[:,1], c=labels, alpha=0.5)
plt.xlabel('od280')
plt.ylabel('malic_acid')
#plt.xticks(range(0,9,2))
#plt.yticks(range(8))
plt.title('Wine Clustering after Standarization')
plt.suptitle(tema)
...
plt.figure()
plt.subplot(1,2,1)
plt.scatter(wine.od280, wine.proline, c=labels, alpha=0.5)
plt.xlabel('od280')
plt.ylabel('malic_acid')
plt.xticks(range(-1000, 1001, 500))
plt.yticks(range(200,1801,200))
plt.title('Wine Clustering')
plt.subplot(1,2,2)
plt.scatter(wine_samples_standarized[:,11], wine_samples_standarized[:,12], c=labels, alpha=0.5)
plt.xlabel('od280')
plt.ylabel('malic_acid')
...
print("****************************************************")
tema = "9. Scaling fish data for clustering"; print("** %s\n" % tema)

scaler = StandardScaler() # Create scaler: scaler
kmeans = KMeans(n_clusters=4) # Create KMeans instance: kmeans
...
print("****************************************************")
tema = "10. Clustering the fish data"; print("** %s\n" % tema)

pipeline.fit(fish_samples) # Fit the pipeline to samples
labels = pipeline.predict(fish_samples) # Calculate the cluster labels: labels

df = pd.DataFrame({'labels': labels, 'species': fish_varieties}) # Create a DataFrame with labels and species as columns: df
ct = pd.crosstab(df['labels'], df['species']) # Create crosstab: ct
...
print("****************************************************")
tema = "11. Clustering stocks using KMeans"; print("** %s\n" % tema)

normalizer = Normalizer() # Create a normalizer: normalizer
kmeans = KMeans(n_clusters=10) # Create a KMeans model with 10 clusters: kmeans
pipeline = make_pipeline(normalizer, kmeans) # Make a pipeline chaining normalizer and kmeans: pipeline

pipeline.fit(movement_samples) # Fit pipeline to the daily price movements



print("****************************************************")
tema = "12. Which stocks move together?"; print("** %s\n" % tema)

labels = pipeline.predict(movement_samples) # Predict the cluster labels: labels
df = pd.DataFrame({'labels': labels, 'companies': movement_varieties}) # Create a DataFrame aligning labels and companies: df
