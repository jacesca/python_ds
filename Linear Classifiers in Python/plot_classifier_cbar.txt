# Author: Mike Gelbart
# Source: https://github.com/mgelbart/plot-classifier/blob/master/plot_classifier/__init__.py
import numpy as np
...
def plot_classifier(X, y, clf, ax=None, ticks=False, proba=False, lims=None, transformation=None, 
                    show_data=True, gray_photocopy=False, proba_showtitle=True, 
                    params_cbar={'fontsize':20, 'labelpad':30}, **kwargs): # assumes classifier "clf" is already fit
    try: 
        X = X.to_numpy()
    except:
        pass
    ...
    if proba == "raw":
        ...
        cbar.ax.set_ylabel('raw model output', rotation=270, **params_cbar)#fontsize=20, labelpad=30)
        #cbar.ax.tick_params(labelsize=14)
    elif proba:
        cbar = plt.colorbar(cs, ax=ax)
        if proba_showtitle:
            cbar.ax.set_ylabel('probability of red $\Delta$ class', rotation=270, **params_cbar)#fontsize=20, labelpad=30)
        #cbar.ax.tick_params(labelsize=14)

    if show_data:
        #ax.scatter(X0, X1, c=y, cmap=plt.cm.coolwarm, s=30, edgecolors='k', linewidth=1)
        if len(labels) == 2:
            ax.scatter(X0[y==labels[0]], X1[y==labels[0]], s=60, c='darkblue', marker='o', edgecolors='k')
            ax.scatter(X0[y==labels[1]], X1[y==labels[1]], s=60, c='darkred', marker='^', edgecolors='k')
        elif len(labels) == 3:
            ax.scatter(X0[y==labels[0]], X1[y==labels[0]], s=60, c='darkblue', marker='o', edgecolors='k')
            ax.scatter(X0[y==labels[1]], X1[y==labels[1]], s=60, c='darkcyan', marker='^', edgecolors='k')
            ax.scatter(X0[y==labels[2]], X1[y==labels[2]], s=60, c='darkred', marker='p', edgecolors='k')
        else:
