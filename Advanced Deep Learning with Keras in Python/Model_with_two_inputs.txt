...
out = Concatenate(name='Gathering-together')([flatten_1, flatten_2, home_in]) # Combine the team strengths with the home input using a Concatenate layer, then add a Dense layer
...
model.fit([games_season['team_1'], games_season['team_2'], games_season['home']], # Fit the model to the games_season dataset
          ...
          epochs=100, verbose=False, validation_split=.10, batch_size=2048, callbacks=[EarlyStopping(patience=2)])
games_tourney['pred'] = model.predict([games_tourney['team_1'], games_tourney['team_2'], games_tourney['home']]) # Predict
print(games_tourney[['score_diff','pred']].head())