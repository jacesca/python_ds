...
x_distro = np.unique(x_distro)
y_distro = population[list(x_distro)]
...
scat, = ax1.plot([], [], ls='None', marker='s', markerfacecolor='darkblue', markersize=3)
samp, = ax1.plot([], [], ls='None', marker='.', markerfacecolor='darkred',  markersize=3)
ax1.legend(['Population', 'Selected Sample'], loc='upper right', fontsize=8)
...
def animate(i):
    ...
    xs = x_distro[x_distro<=i]; ys = y_distro[np.where(x_distro<=i)]
...
#anim.save('04_05_Second_Animation.gif', writer=writer)
