...
###############################################################################
# Fourth animation
subtopic = "Fourth animation..."; print("** %s\n" % subtopic)
...
def animate(i):
    x = x_distro[np.where(n_distro==i)]
    y = y_distro[np.where(n_distro==i)]
    
    scat.set_data(x, y)
    return scat, 

## (4) Start the animation.
anim = FuncAnimation(fig, animate, init_func=init, frames=n_sample_means, interval=100, blit=False, repeat=True, repeat_delay=100)
plt.show()
